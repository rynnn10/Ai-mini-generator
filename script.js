...
// --- API KEY KONFIGURASI (TERTANAM SESUAI PERMINTAAN) ---
const AI_CONFIG = {
...
// --- 4. CONVERTER LOGIC (UNTOUCHED - Fixed NaN) ---
let currencyRates = {
...
// --- 5. AI CHAT LOGIC (UPDATED from Script.js) ---
...
async function sendMessage() {
    ...
    try {
        ...
        if (!response.ok) {
            // Jika error 404/Not Found pada model 2.5
            if(data.error?.code === 404 || data.error?.message?.includes('not found')) {
                throw new Error("Model Gemini 2.5 Flash belum aktif untuk API Key ini. Coba lagi nanti.");
            }
